...
<%
	...
	try
	{
	    ...
	    WhereCondition=" where parcode=5 ";
...
<!--
	...
	function goView(thid,PAGE){
		...
		pbform.action = "budget_view.jsp";
		pbform.submit();
	}
	function goDetailView(thid,PAGE){
		pbform.thid.value=thid;
		pbform.PAGE.value=PAGE;
		pbform.action = "budget_detail.jsp";
		pbform.submit();
	}
...
<form method=post name="pbform" action="" style="margin:0px;">
    ...
    <div class="list-header">
        <table border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td rowspan="2" class="admin_subject">예산낭비신고</td>
    ...
    <div class="tbl-wrap">
      ...
      <table border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td height="30" align="right"><a href="excel/corruption_excel.jsp?cmd=<%=cmd%>&KEY=<%=KEY%>&FIELD=<%=FIELD%>&startday=<%=startday%>&endday=<%=endday%>&status=<%=status%>&buse_name=<%=buse_name%>&result=<%=result%>"><img src="img/btn_excel.gif" width="66" height="21" alt="엑셀" border="0"></a></td>
        </tr>
      </table>
      <!-- // 검색결과 -->
    <div class="subtit"><img src="img/arrow.gif" width="13" height="16" align="absmiddle">예산낭비 신고</div>
      ...
      <table width="945" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td align="center">
			...
			<%=FrontDbUtil.PrintPage(Integer.toString(PAGE), TotalRecordCount, DefaultListRecord, DefaultPageBlock, "budget.jsp?KEY="+KEY+"&cmd=" + cmd + "&FIELD=" + FIELD + "&startday=" + startday + "&endday=" + endday + "&status=" + status + "&buse_name=" + buse_name + "&PAGE=","<img src=img/board_prev.gif width=13 height=13 align=absmiddle border=0>","<img src=img/board_next.gif width=13 height=13 align=absmiddle border=0>")%>
			<% } %>
          </td>
        </tr>
      </table>
...
<%
		...
		out.println("budget.jsp : " + e.getMessage());
	} finally {
