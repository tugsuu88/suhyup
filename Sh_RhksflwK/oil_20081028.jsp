...
<%
	...
	{	    
		...
		if(!finance_buseo.equals("감사실") &&  !finance_buseo.equals("준법감시실")){
        ...
        <tr> 
          <td width="135"> 
            <table width="135" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td height="70" align="center" class="admin_subject">면세유류<br>부정유출신고</td>
              </tr>
            </table>
          </td>
          <td width="5"></td>
                    ...
                    <tr> 
                      ...
                      <td width="100"> 
                        <select name="buse_name">
                          <option value="">선택</option>
						  <option value="가맹사업단"<%if(buse_name.equals("가맹사업단")) { %>selected<% } %>>가맹사업단</option>
						  <option value="감사실"<%if(buse_name.equals("감사실")) { %>selected<% } %>>감사실</option>
						  <option value="개인고객부"<%if(buse_name.equals("개인고객부")) { %>selected<% } %>>개인고객부</option>
						  <option value="개인고객부(전자금융)"<%if(buse_name.equals("개인고객부(전자금융)")) { %>selected<% } %>>개인고객부(전자금융)</option>
						  <option value="IT관리실"<%if(buse_name.equals("IT관리실")) { %>selected<% } %>>IT관리실</option>
						  <option value="경영지원부"<%if(buse_name.equals("경영지원부")) { %>selected<% } %>>경영지원부</option>
						  ...
						  <option value="기업고객부"<%if(buse_name.equals("기업고객부")) { %>selected<% } %>>기업고객부</option>
						  <option value="기획관리부"<%if(buse_name.equals("기획관리부")) { %>selected<% } %>>기획관리부</option>
						  ...
						  <option value="바다마트사업부"<%if(buse_name.equals("바다마트사업부")) { %>selected<% } %>>바다마트사업부</option>
						  <option value="상호금융부"<%if(buse_name.equals("상호금융부")) { %>selected<% } %>>상호금융부</option>
						  <option value="수매사업단"<%if(buse_name.equals("수매사업단")) { %>selected<% } %>>수매사업단</option>
						  ...
						  <option value="외환사업실"<%if(buse_name.equals("외환사업실")) { %>selected<% } %>>외환사업실</option>
						  ...
						  <option value="전략마케팅실"<%if(buse_name.equals("전략마케팅실")) { %>selected<% } %>>전략마케팅실</option>
						  <option value="전산정보부"<%if(buse_name.equals("전산정보부")) { %>selected<% } %>>전산정보부</option>
						  <option value="조합감사실"<%if(buse_name.equals("조합감사실")) { %>selected<% } %>>조합감사실</option>
						  <option value="조합금융리스크관리실"<%if(buse_name.equals("조합금융리스크관리실")) { %>selected<% } %>>조합금융리스크관리실</option>
						  <option value="조합자금부"<%if(buse_name.equals("조합자금부")) { %>selected<% } %>>조합자금부</option>
						  ...
						  <option value="특판사업부"<%if(buse_name.equals("특판사업부")) { %>selected<% } %>>특판사업부</option>
						  <option value="판매사업부"<%if(buse_name.equals("판매사업부")) { %>selected<% } %>>판매사업부</option>
						  <option value="해양투자금융부"<%if(buse_name.equals("해양투자금융부")) { %>selected<% } %>>해양투자금융부</option>
						  ...
						  <option value="FS사업부"<%if(buse_name.equals("FS사업부")) { %>selected<% } %>>FS사업부</option>
						  <option value="직판사업단"<%if(buse_name.equals("직판사업단")) { %>selected<% } %>>직판사업단</option>
                          <!-- -->
                        </select>
                      </td>
                      <td width="20">&nbsp;</td>
    ...
    <td style="padding:0 0 10 10 ;"> 
      ...
      <table width="960" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          ...
          <td height="30" align="right"><a href="excel/oil_excel.jsp?cmd=<%=cmd%>&KEY=<%=KEY%>&FIELD=<%=FIELD%>&startday=<%=startday%>&endday=<%=endday%>&status=<%=status%>&buse_name=<%=buse_name%>&result=<%=result%>"><img src="img/btn_excel.gif" width="66" height="21" alt="엑셀" border="0"></a></td>
        </tr>
        <tr> 
