	...
	if(ResultVector.size() > 0){
		for (int i = 0; i < ResultVector.size(); i++) {
			...
			if(i == 0){
				...
				if("감사실".equals(finance_buseo)){ /* buseo */
				 	/* session.setAttribute("buseName", "수협중앙회-자회사"); */
					if("1".equals(bupin)){
					 	session.setAttribute("buseName", "수협중앙회-자회사");
					}
					if("2".equals(bupin)){
					 	session.setAttribute("buseName", "수협은행");
					}
					if("3".equals(bupin)){
					 	session.setAttribute("buseName", "-회원조합");
					}
				}else{
					if("준법감시실(중앙회)".equals(finance_buseo)){ /* buseo */
					 	session.setAttribute("buseName", "수협중앙회-자회사");
					}
					if("금융소비자보호부".equals(finance_buseo)){
					 	session.setAttribute("buseName", "수협은행");
					}
					if("준법감시실(조합)".equals(finance_buseo)){
					 	session.setAttribute("buseName", "-회원조합");
					}
				}
			}
...
<script language="javascript">
	...
	function uploadFile_Change( value ){
    	...
    	/* if (src == "") {
        	...
        	alert("hwp 와 ppt, xls 파일만 첨부 가능합니다.");
        	document.pbform.file1.value = "";
        	//pbform.afile.focus();
        	return;
    	} */
	...
	function send() {
		...
		if (pbform.buseo.value == '회원조합') {
			if(!pbform.sh_johap.value) {
				alert("이첩할 조합명을 선택해주세요");
				pbform.sh_johap.focus();
				return;
			}
	...
	function buseoChange(value) {		
		if(value == "회원조합") {
			pbform.sh_johap.style.display = 'block';
		}
		else {
			pbform.sh_johap.style.display = 'none';
		}
	}
</script>
<!-- 금융사고 제보 설명 -->
<form method=post name="pbform" action="" style="margin: 0px;">
	<input type="hidden" name="bupin" value="<%=bupin%>" />
				...
				<table width="965" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td height="30" align="right" width="60"><a href="#"><img
								...
								onClick="MM_openBrWindow('minwon_03_print2.jsp?thididx=<%=thididx%>&bupin=<%=bupin%>&yongup=<%=yongup%>','','width=638,height=630')"
											...
											<tr>
												...
												%>
													...
													%> <a href="javascript:DownloadPopup('<%=mid%>');">
															<img src="img/file_icon.gif" border="0"> <%=fname %>
														</a> 
													<%
 	}
 %>
												...
												<td valign="top">
													<table width="100%" border="0" cellspacing="0"
														...
														<tr>
															...
															<%
																if (buse_name.equals("감사실") || 
																	buse_name.equals("감사부") ||
																	"준법감시실(조합)".equals(buse_name) || 
																	"준법감시실(중앙회)".equals(buse_name)) {
															%>
															<td>
																...
																<%
																	if (((finance_buseo.equals("감사실") || finance_buseo.equals("감사부")) && answer.equals("") && !status.equals("철회완료")) ||
																		((finance_buseo.equals("준법감시실(조합)") && answer.equals("") && !status.equals("철회완료") || finance_buseo.equals("준법감시실(중앙회)")) && answer.equals("") && !status.equals("철회완료")) ) {
																%>
																<select name="buseo" onchange="javascript:buseoChange(this.value);">
														...
														%>
															...
															<option value="<%=sh_buseo_cd%>">
																<%=sh_buseo_nm%>
															</option>
														
															<% } %>
														<% } %>
																<option value="회원조합">회원조합</option>
															</select> 
															<select name="sh_johap" id=""  style="margin-top:10px;margin-bottom:10px;display:none">
																<option value="">선택</option>
														<%										
														
															String TableNameJohap = " sh_johap ";
															String SelectConditionJohap = "JOHAP_NAME";
															String WhereConditionJohap = "where 1 = 1 ";
															String OrderConditionJohap = "";
															
															ResultVector = FrontBoard.list(TableNameJohap, SelectConditionJohap, WhereConditionJohap, OrderConditionJohap, 1, 0, 100);
														...
														<% } %>
															...
															<a href="javascript:receive();">
															 ...
															 --%>
															 <input type="hidden" name="buseo_to" value="<%=buse_name%>">
															...
															%>
																<td>금융소비자보호단으로
																	<a href="javascript:receive();"><img src="img/btn_sendback.gif" align="absmiddle" border="0" alt="반송"></a>
